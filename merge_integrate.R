...
tmp_colours <- new_colour_scheme[new_colour_scheme[,1] %in% merged_obj@meta.data$short_cluster_anno,]
png("merged_not_integrated_umap_anno.png", width=9, height =6, units="in", res=50)
DimPlot(merged_obj, reduction="umap_no_harmony", group.by="short_cluster_anno", pt.size=.1)+scale_color_manual(values=tmp_colours[,2])
dev.off();
png("merged_not_integrated_tsne_anno.png", width=9, height =6, units="in", res=50)
DimPlot(merged_obj, reduction="tsne_no_harmony", group.by="short_cluster_anno", pt.size=.1)+scale_color_manual(values=tmp_colours[,2])
dev.off();
require("harmony")
set.seed(10131)
...
DimPlot(merged_obj, reduction="umap_harmony", group.by="short_cluster_anno", pt.size=.1)+scale_color_manual(values=tmp_colours[,2])
dev.off();
png("merged_harmony_integrated_tsne_anno.png", width=9, height =6, units="in", res=50)
DimPlot(merged_obj, reduction="tsne_harmony", group.by="short_cluster_anno", pt.size=.1)+scale_color_manual(values=tmp_colours[,2])
dev.off();
